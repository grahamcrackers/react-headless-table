...
const TableWithSelection = <T extends DataType>({ columns, data }: { columns: ColumnType<T>[]; data: T[] }) => {
    const { headers, rows, selectRow, selectedRows, toggleAll } = useTable(columns, data, {
        selectable: true,
    });
...
const TableWithSelectionAndFiltering = <T extends DataType>({
                ...
                return rows.filter((row) => {
                    return (
                        row.cells.filter((cell) => {
                            ...
                            return cell.value.toLowerCase().includes(searchString) ? true : false;
                        }).length > 0
                    );
                });
            },
    ...
    return (
        <>
            ...
            ></input>
            <button data-testid="filter-button" onClick={() => setFilterOn(!filterOn)}>
                Filter
            </button>
            <table>
                <thead>
