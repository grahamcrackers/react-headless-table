...
const Table = ({ columns, data }: { columns: any[]; data: { firstName: string; lastName: string }[] }) => {
    const { headers, rows } = useTable<{ firstName: string; lastName: string }>(columns, data);
...
test('Should update table rows when data changes', () => {
    ...
    const newData = [...data, { firstName: 'Bilbo', lastName: 'Baggins' }];
