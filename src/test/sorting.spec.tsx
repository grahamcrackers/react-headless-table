...
const Table = <T extends {}>({ columns, data }: { columns: ColumnType<T>[]; data: T[] }) => {
    ...
    return (
        <>
                ...
                <thead>
                    <tr>
                        {headers.map((header, idx) => (
                            <th key={idx} data-testid={`column-${header.name}`} onClick={() => toggleSort(header.name)}>
                                {header.label}
            ...
            <button
                data-testid="add-row"
                onClick={() => setStateDate([...data, { ...data[0], firstName: 'new', lastName: 'person' }])}
            >
                Add Row
            </button>
        </>
...
test('Should sort by dates correctly', () => {
    ...
    columns[2] = {
        ...
        sort: (objectA, objectB) => {
            return Number(new Date(objectA.original.birthDate)) - Number(new Date(objectB.original.birthDate));
        },
    };
